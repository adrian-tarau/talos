...
import static net.microfalx.talos.extension.MavenUtils.*;
...
public class ProfilerMetrics {
    ...
    void print() {
        ...
        printFailureSummary();
        if (shouldShowLineSeparator()) infoLine('-');
        if (configuration.isQuiet() && net.microfalx.talos.core.MavenUtils.isMavenLoggerAvailable()) {
    ...
    private String getRepositoriesInfo() {
        return "Local: " + session.getLocalRepository().getBasedir() + ", Remote: " +
               net.microfalx.talos.core.MavenUtils.getRemoteRepositories(session).stream()
    ...
    private String formatDuration(Duration duration) {
        return buffer().strong(net.microfalx.talos.core.MavenUtils.formatDuration(duration, false, false)).toString();
    }
